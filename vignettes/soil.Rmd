...
For the contents of these color charts we depend completely on Ferguson @Ferguson2014.
...
we call this name the `FergusonName`.
...
In the following function `soil_data()`, argument `soil_spec` is a `colorSpec` object with
reflectance spectra of the soil, and `CCT` is the desired Correlated Color Temperature of the illuminant.

<button id="B-soil_data()" onclick="ToggleDiv('soil_data()')">Show soil_data()</button>
<div id="D-soil_data()" style="display: none">
```{r  echo=TRUE, warning=TRUE, results="hide" }
...
and the `FergusonName`s are computed from `MunsellRounded`.
...
<button id="B-table code" onclick="ToggleDiv('table code')">Show table code</button>
<div id="D-table code" style="display: none">
```{r echo=TRUE, message=TRUE}
