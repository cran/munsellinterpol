...
ColorBlockFromMunsell  <-  function( MunsellSpec )
    ...
    if( is.character(MunsellSpec) )
        ...
        #  this WARN is not needed, the WARN will come from HVCfromMunsellName()
        #mask    = is.na( HVC[ ,1] )
        #if( any(mask) )
        #    log_level( WARN, "%d (out of %d) Munsell notations are invalid.", sum(mask), nrow(HVC) )
    ...
    for( i in 1:n )
        ...
        if( nrow(theRow) != 1 )
            {
            event_level( WARN, "(internal table error).  Expected exactly 1 match for HVC=%g,%g,%g, but found %d.\n",
                                    hvc[1],hvc[2],hvc[3], nrow(theRow),
                                    class = "table_error", extra = list(indexes=i) )
            next
            }
    ...
    if( any(bad) )
        {
        event_level( WARN, "%d, of %d, color blocks could not be computed.", sum(bad), length(bad),
                        class = "incomplete", extra = list( indexes=which(bad)) )
        }
