...
IsWithinMacAdamLimits <- function( xyY, Illuminant='C' )
    ...
    if( is.na(idx) )
        {
        event_level( ERROR, "Argument Illuminant='%s' is invalid.", Illuminant,
                                class="invalid_argument", extra=list(Illuminant=Illuminant) )
        return(NULL)
        }
    Illuminant  = full[idx]

    zono    = p.OptimalHull[[ Illuminant ]]

    #   out = is.finite( geometry::tsearchn( hull$XYZ, hull$tessellation, spacesXYZ::XYZfromxyY(xyY) )$idx )

    out = inside( zono, spacesXYZ::XYZfromxyY(xyY) )
...
sectionOptimals  <-  function( Y, n=100, illum='C' )
    ...
    if( ! ok )
        {
        event_level( WARN, "Y=%g is invalid; it must be in interval (0,100).", Y,
    ...
    if( ! ok )
        {
        event_level( ERROR, "Argument illum='%s' is invalid.", illum,
                            class="invalid_argument", extra=list(illum=illum) )
        return(NULL)
        }
    ...
    if( ! identical( wave, p.xyz1931$Wavelength ) )
        {
        event_level( FATAL, "Internal Error. illuminant '%s' and xyz have different wavelengths.", illum,
