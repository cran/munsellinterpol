...
NickersonColorDifference <- function( HVC0, HVC1, symmetric=TRUE, coeffs=c(0.4,6,3) )
    ...
    if( nrow(HVC0) != nrow(HVC1) )
        {
        event_level( ERROR, "nrow(HVC0) = %d != %d = nrow(HVC1).", nrow(HVC0), nrow(HVC1),
    ...
    if( ! ok )
        {
        event_level( ERROR, "argument 'coeffs' is invalid; it must have length 3 and all positive.",
                            class="invalid_argument", extra=list(coeffs=coeffs) )
        return(NULL)
        }
    ...
    if( any(bad) )
        {
        event_level( WARN, "%d, of %d, color differences could not be computed.", sum(bad), length(bad),
