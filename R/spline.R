...
bicubicCardinal  <- function( x, y, z, x0, y0 )
    ...
    if( lenx != nrow(z) )
        {
        event_level( FATAL, "Internal Error. length(x)=%d != %d=nrow(z)", length(x), nrow(z),
    ...
    if( leny != ncol(z) )
        {
        event_level( FATAL, "Internal Error. length(y)=%d != %d=ncol(z)", length(y), ncol(z),
                                class="internal_error" )
        return(NULL)
        }
    ...
    if( length(x0) != n )
        {
        event_level( FATAL, "Internal Error. length(x0)=%d != %d=length(y0)", length(x0), n,
                                class="internal_error" )
        return(NULL)
        }
...
splineCardinal <- function( x, y, xout )
    ...
    if( nrow(y) != n )
        {
        event_level( FATAL, "length(x) = %d  !=  %d = nrow(y).", length(x), nrow(y),
                                class="internal_error" )
        return(NULL)
        }
