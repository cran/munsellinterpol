...
makeReparamFunctionList <- function( .lambda, .illum, .xyz, .mode='equalize', .pnorm=2, .Ymax=1 )
    ...
    if( length(.illum) != n )
        {
        event_level( FATAL, "Internal Error. length(.illum) = %d is incorrect.", length(.illum),
    ...
    if( ! all( dim(.xyz) == c(n,3) ) )
        {
        event_level( FATAL, "Internal Error. dim(.xyz) = %d,%d is incorrect.", dim(.xyz)[1], dim(.xyz)[2],
                                class="internal_error" )
        return(NULL)
        }
    ...
    if( length(step.wl) != 1 )
        {
        event_level( FATAL, "Internal Error. %d wavelengths are not regular.", length(.lambda),
                                    class="internal_error" )
        return(NULL)
        }
    ...
    if( rank <  min(dim(coredata)) )
        {
        event_level( FATAL, "Internal Error. The responsivity matrix is rank-deficient (rank=%d < %d).", rank, min(dim(coredata)),
                                    class="internal_error" )
        return(NULL)
        }
    ...
    if( .mode == 'equalize' )
        ...
        if( any(bad) )
            {
            event_level( FATAL, "Internal Error. Responder has responsivity=0 at %d wavelengths, of %d, which is invalid.",
                                    sum(bad), length(bad), class="internal_error" )
            return(NULL)
            }
        ...
        {
        event_level( FATAL, "Internal Error. .mode='%s' is invalid.", .mode, class="internal_error" )
        return(NULL)
        }
