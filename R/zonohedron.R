...
#
#   zonohedron()        this is the constructor, which is only called from savePrivateDatasets().
#                       It is called for 2 illuminants - "C" and "D65" - and
#                       the two resulting zonohedra are stored in sysdata.rda.
#                       Since this is never called by the user,
#                       just leave log_level(ERROR,...); event_level(ERROR,...) not necessary
#
#   inside.zonohedron() only called from IsWithinMacAdamLimits().
#                       event_level( ERROR,...) might be called from prepareNx3().
...
zonohedron  <- function( W, tol=1.e-6 )
    ...
    idx = t( utils::combn(n,2) )   # matrix of pairs.  This is kind of slow, but OK for only 81 wavelengths
    ...
    if( any(bad) )
        {
        #mess    = sprintf( "WARN.  %d bad normals, out of %d.\n", sum(bad), length(bad) )
        #cat(mess)
        log_level( WARN, "%d normals, out of %d, flagged as too small.", sum(bad), length(bad) )
        normal[ bad, ]  = NA_real_
        }
...
#           within  logical
#           delta   numeric, non-positive means within is TRUE
