...
addPredictions <- function( data, warn=TRUE )
    ...
    if( warn  &&  any( is.na(out$A.pred) ) )
        event_level( WARN, "%d of A.pred are NA.", sum(is.na(out$A.pred)) )
    ...
    if( warn  &&  any( is.na(out$x.pred) ) )
        event_level( WARN, "%d of x.pred are NA.", sum(is.na(out$x.pred)) )
    ...
    if( warn  &&  any(mask) )
        {
        event_level( WARN, "%d of a.pred are NA.", sum(mask) )
        print( out[mask, ] )
        }
